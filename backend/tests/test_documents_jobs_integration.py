...
def _upload_text(client: TestClient, content: bytes, filename: str = "doc.txt") -> dict:
...
def _poll_job_until_terminal(client: TestClient, job_id: str, timeout_sec: float = 5.0) -> tuple[dict, list[str]]:
    deadline = time.time() + timeout_sec
    seen_stages: list[str] = []
    last_payload: dict | None = None

    while time.time() < deadline:
        response = client.get(f"/api/v1/documents/jobs/{job_id}", headers=_auth_headers())
...
def test_documents_jobs_success_pipeline_to_ready_and_chunks(monkeypatch) -> None:
    with TestClient(app) as client:
        ...
        accepted = _upload_text(client, b"line1\nline2\nline3")
        job_id = accepted["job_id"]
        document_id = accepted["document_id"]

        final_job, seen_stages = _poll_job_until_terminal(client, job_id)
        ...
        docs_resp = client.get("/api/v1/documents", headers=_auth_headers())
        assert docs_resp.status_code == 200
        docs_payload = docs_resp.json()
        doc = next(item for item in docs_payload["items"] if item["document_id"] == document_id)
        assert doc["status"] == "ready"

        chunks_resp = client.get(f"/api/v1/documents/{document_id}/chunks", headers=_auth_headers())
...
def test_documents_jobs_failure_sets_document_failed_and_error_code() -> None:
    with TestClient(app) as client:
        accepted = _upload_text(client, b"   \n\n\t\n", filename="empty.txt")
        job_id = accepted["job_id"]
        document_id = accepted["document_id"]

        final_job, _ = _poll_job_until_terminal(client, job_id)
...
def test_documents_jobs_cancel_running_and_finished_conflict(monkeypatch) -> None:
    with TestClient(app) as client:
        ...
        accepted = _upload_text(client, b"cancel-me")
        job_id = accepted["job_id"]
        document_id = accepted["document_id"]

        cancel_resp = client.post(f"/api/v1/documents/jobs/{job_id}/cancel", headers=_auth_headers())
        ...
        list_by_status = client.get("/api/v1/documents/jobs", params={"status": "canceled"}, headers=_auth_headers())
        ...
        list_by_doc = client.get("/api/v1/documents/jobs", params={"document_id": document_id}, headers=_auth_headers())
        assert list_by_doc.status_code == 200
        doc_items = list_by_doc.json()["items"]
        assert any(item["job_id"] == job_id for item in doc_items)

        # 已完成任务不可取消
        accepted2 = _upload_text(client, b"done")
        succeeded_job, _ = _poll_job_until_terminal(client, accepted2["job_id"])
        assert succeeded_job["status"] == "succeeded"

        conflict_resp = client.post(f"/api/v1/documents/jobs/{accepted2['job_id']}/cancel", headers=_auth_headers())
...
def test_documents_jobs_list_pagination_boundaries() -> None:
    with TestClient(app) as client:
        ...
        for idx in range(5):
            accepted = _upload_text(client, f"doc-{idx}".encode("utf-8"), filename=f"d{idx}.txt")
            created_job_ids.append(accepted["job_id"])

        for job_id in created_job_ids:
            _poll_job_until_terminal(client, job_id)

        page1 = client.get("/api/v1/documents/jobs", params={"page": 1, "page_size": 2}, headers=_auth_headers())
        ...
        page3 = client.get("/api/v1/documents/jobs", params={"page": 3, "page_size": 2}, headers=_auth_headers())
        ...
        beyond = client.get("/api/v1/documents/jobs", params={"page": 999, "page_size": 2}, headers=_auth_headers())
...
def test_documents_jobs_batch_build_returns_partial_failures_without_swallowing() -> None:
    with TestClient(app) as client:
        ok_doc = _upload_text(client, b"ok-batch", filename="ok-batch.txt")
        _poll_job_until_terminal(client, ok_doc["job_id"])
