...
class DocumentService:
    ...
    @staticmethod
    def _storage_key(document_id: str, filename: str) -> str:
        safe_name = Path(filename or "unknown").name or "unknown"
    ...
    async def create_upload_job(self, filename: str, file_type: str, content: bytes) -> dict[str, Any]:
        ...
        now = self._now()
        storage_path: str | None = None
        inline_content: bytes | None = content

        if self._storage is not None:
            try:
                storage_path = await self._storage.save(key=self._storage_key(document_id, filename), content=content)
                inline_content = None
        ...
        await self._queue_runner.enqueue("document.build", payload)
        return {
            "job_id": job_id,
            "document_id": document_id,
    ...
    async def hybrid_retrieve(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        ...
        for item in merged.values():
            ...
            fused_score = dense_score * self._tuning.dense_weight + sparse_score * self._tuning.sparse_weight
        ...
        for document_id in document_ids:
            ...
            if storage_path and self._storage is not None:
                try:
                    ...
                    logger.exception("document.storage.delete_failed", extra={"document_id": document_id})
    ...
    async def _read_document_bytes(self, doc: DocumentRecord) -> bytes:
        if doc.content is not None:
            return doc.content
        if doc.storage_path and self._storage is not None:
            try:
                return await self._storage.read(doc.storage_path)
            except Exception as exc:
                raise AppError(
                    "DOC_STORAGE_ERROR",
        ...
        raise AppError("DOC_STORAGE_ERROR", "Document content unavailable", status_code=500)
    ...
    async def _parsing(self, job_id: str) -> str:
        ...
        text = (await self._read_document_bytes(doc)).decode("utf-8", errors="ignore").strip()
        if not text:
...
class DocumentTaskExecutor:
    ...
    async def execute(self, task_name: str, payload: dict) -> None:
        ...
        if payload_request_id:
            request_token = request_id_ctx.set(str(payload_request_id))
